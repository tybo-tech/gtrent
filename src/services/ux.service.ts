...
import { HomeNavUx, LoaderUx, NavHistoryUX, PopUx } from 'src/models/UxModel.model';
...
export class UxService {
  ...
  private uxMessagePopBehaviorSubject: BehaviorSubject<PopUx>;
  public uxMessagePopObservable: Observable<PopUx>;
  ...
  constructor() {
    this.uxMessagePopBehaviorSubject = new BehaviorSubject<PopUx>(null);
  ...
  public get currentMessagePopValue(): PopUx {
  ...
  updateMessagePopState(mesg: string = '', state: PopUx = null) {
    if (state) {
      this.uxMessagePopBehaviorSubject.next(state);
    }
    if (!state && mesg) {
      this.uxMessagePopBehaviorSubject.next({ Message: mesg, Class: '' });
