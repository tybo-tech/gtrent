...
export class UploadService {
  ...
  resizeImage(file, product: Product = null, user: User = null, variationOption: VariationOption = null, promaotion: Promotion = null, compressor: Machine = null) {
      ...
      reader.onload = (readerEvent: any) => {
        ...
        image.onload = (imageEvent) => {
          ...
          this.uploadFile(formData).subscribe(response => {
            this.doneUplaoding(response, product, user, variationOption, promaotion,compressor);
    ...
    if (file.type.match(/image.*/) && file.type === 'image/gif') {
      ...
      this.uploadFile(formData).subscribe(url => {
        this.doneUplaoding(url, product, user, variationOption, promaotion,compressor);
      });
    }
  }
  doneUplaoding(response: any, product, user, variationOption, promaotion,compressor: Machine) {
    if (response) {
      if (product) {
      ...
      if (compressor) {
        compressor.Moreinfo1 = `${environment.API_URL}/api/upload/${response}`;
      }
    }
  }
